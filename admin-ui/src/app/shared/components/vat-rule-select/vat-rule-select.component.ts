...
import {VatRuleApiService} from '../../../features/settings/accounting/services/vat-rule-api.service';
import {NgLabelTemplateDirective, NgOptionTemplateDirective, NgSelectComponent} from '@ng-select/ng-select';
import {TranslatePipe} from '@ngx-translate/core';
import {DecimalPipe} from '@angular/common';

@Component({
  selector: 'app-vat-rule-select',
  imports: [
  ...
  templateUrl: './vat-rule-select.component.html',
  styleUrl: './vat-rule-select.component.scss',
  standalone: true,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: forwardRef(() => VatRuleSelectComponent),
    }
  ]
})
export class VatRuleSelectComponent implements OnInit, OnDestroy, ControlValueAccessor {
  private component = 'VatRuleSelectComponent: ';
  ...
  constructor(private readonly vatRuleApiService: VatRuleApiService) {
  }
  ...
  private retrieveResourceSearchList() {
    ...
    this.subscriptions.push(
      this.vatRuleApiService.getAllByPage(pageFilter).subscribe({
        next: (res) => {
