...
import {
  CancellationPolicyApiService
} from '../../../features/settings/policies/cancellations/services/cancellation-policy-api.service';

@Component({
  selector: 'app-cancellation-policy-select',
  imports: [
    BadgeComponent,
  ...
  templateUrl: './cancellation-policy-select.component.html',
  standalone: true,
  styleUrl: './cancellation-policy-select.component.scss',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: forwardRef(() => CancellationPolicySelectComponent),
    }
  ]
})
export class CancellationPolicySelectComponent implements OnInit, OnDestroy, ControlValueAccessor {
  private component = ['CancellationPolicySelectComponent'];
  ...
  constructor(private readonly cancellationPolicyApiService: CancellationPolicyApiService) {
  }
  ...
  private retrieveResourceSearchList() {
    ...
    this.subscriptions.push(
      this.cancellationPolicyApiService.getAllByPage(pageFilter).subscribe({
        next: (res) => {
