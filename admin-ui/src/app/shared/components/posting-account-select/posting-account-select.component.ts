...
import {PostingAccountApiService} from '../../../features/settings/accounting/services/posting-account-api.service';

@Component({
  selector: 'app-posting-account-select',
  imports: [
  ...
  templateUrl: './posting-account-select.component.html',
  styleUrl: './posting-account-select.component.scss',
  standalone: true,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: forwardRef(() => PostingAccountSelectComponent),
    }
  ]
})
export class PostingAccountSelectComponent implements OnInit, OnDestroy, ControlValueAccessor {
  private component = ['PostingAccountSelectComponent'];

  @Input() disable = false;
  @Input() initialResource: any | null = null;
  @Output() updatedResource = new EventEmitter<any | null>();
  private _accountClass: any
  ...
  constructor(private readonly postingAccountApiService: PostingAccountApiService) {
  }
  ...
  private retrieveResourceSearchList() {
    ...
    let pageFilter: PageFilterModel = {
      ...
      advancedSearchFormValue: {
        accountClass: this.accountClass ? this.accountClass : undefined,
        enabled: true
      }
    };

    this.subscriptions.push(
      this.postingAccountApiService.getAllByPage(pageFilter).subscribe({
        next: (res) => {
